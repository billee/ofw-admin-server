...
if not firebase_admin._apps:
    try:
        # Try to use Firebase credentials from environment variable first (production)
        ...
        if firebase_creds:
            ...
            app.logger.info("✅ Firebase initialized from environment variable")
        elif os.path.exists('serviceAccountKey.json'):
            # Fallback for local development only
            cred = credentials.Certificate('serviceAccountKey.json')
            firebase_admin.initialize_app(cred)
            app.logger.info("✅ Firebase initialized from local file (development)")
        else:
...
# ============================================================================
# CHAT AND AI ROUTES (for mobile app)
# ============================================================================
...
@app.route('/health')
def health_check():
    return jsonify({"status": "healthy", "message": "OFW Flask server is running"})
...
def get_trial_status(trial_history):
    ...
    try:
        if hasattr(trial_end, 'timestamp'):
            end_date = trial_end.replace(tzinfo=None)
